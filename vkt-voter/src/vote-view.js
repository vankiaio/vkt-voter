...
class VoteView extends ModalStackMixin {
    constructor(vnode) {
      ...
      ScatterJS.scatter.connect('VKT-VOTER', globals.connectionOptions).then(connected => {
          if(!connected) {
                  ...
                  //console.log('Calling get identity returned=', identity);
                    ...
                    if (identity.accounts[0].authority != 'active'){
                        this.push_modal([ErrorModal, {owner: this, error_messages: ['You have chosen an account with the ' + identity.accounts[0].authority +
                              ' authority only the active authority can stake VKT. You should change identity'], show_retry: true}]);
                        m.redraw();
                        return;
                    }
                    ...
                    eos.getAccount({'account_name': identity.accounts[0].name}).then((result) => {
                            ...
                            eos.getTableRows({
                                ...
                                (result) => {
                                    const row = result.rows.find(row => row.balance.split(" ")[1].toLowerCase() === 'vkt');
  ...
  vote_for_eosphere() {
    if (globals.proxy_name != '')
      return m('p.header-info-text', 'Service provided by vankia');
    //let bp_name = 'lioninjungle';
    let bp_name = 'eosphereiobp';
    if (globals.votes.includes(bp_name))
      return m('p.header-info-text', 'Thanks for voting for vankia');
    else
      return m('p.header-info-text', [
        'Service provided by vankia ',
        ...
        return m("main", [
                 m("div", {'class': 'pageheader'}, [
                   ...
                   m("div", {'class': 'centre-header'}, [
                     m("h1", {'class': 'centre-h1'}, "VKT Voter")
                   ]),
                 ...
                 m("div", {'class': 'content-container'}, [
                   ...
                   /*
                   m("p.centre", 'Percentage of VKT voting ' + globals.activated_percent + '%'),
                   m("p.centre", Humanize.formatNumber(globals.total_activated_stake) + ' VKT have voted'),
                   */
                   m("p.centre-activated", 'vkt-voter now supports scatter desktop.'),
               ...
               m('p', [
                 ...
                 m('a', {'href':'https://eosphere.io', 'target':'_blank'}, 'vankia'),
                 ' Source code licenced under Affero GNU GPL ',
                 m('a', {'href': 'https://github.com/eosphere/vkt-voter', 'target':'_blank'},
                 'Download source'),
               ]),
             ]),
        )
